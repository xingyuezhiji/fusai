...
x = train[predictors]
y = train['label']
kf = KFold(train.shape[0],n_folds=5,shuffle=True,random_state=100)
#划分训练集和测试集
...
for tr, te in kf:
    ...
    test_ensemble = []
    # test_ensemble = test_ensemble.tolist()
    for clf in classifiers:
        ...
        train_predictions = clf.predict(train[predictors].iloc[te])
    ...
    # print(np.shape(np.sum(ensemble,axis=0)))
...
# print((np.sum(accu)-np.min(accu))/4)
print(np.mean(accu))
